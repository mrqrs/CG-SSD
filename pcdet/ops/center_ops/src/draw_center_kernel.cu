...
__global__ void draw_aux_kernel(int batch_size, int max_boxes, int max_objs, int num_cls, int H, int W, int code_size, int min_radius,
                                    float voxel_x, float voxel_y, float range_x, float range_y, float out_factor, float gaussian_overlap,
                                    const float *gt_boxes, float *heatmap, int *gt_ind, int *gt_mask, int *gt_cat, float *gt_box_encoding, int *gt_cnt){
    ...
    */
    ...
    gt_mask += bs_idx * num_cls * max_objs;
    gt_cat += bs_idx * num_cls * max_objs;
    gt_box_encoding += bs_idx * num_cls * max_objs * 2;
    gt_cnt += bs_idx * num_cls;

    // gt box parameters
    float x = gt_boxes[box_idx * code_size + 0];
    float y = gt_boxes[box_idx * code_size + 1];
    float dx = gt_boxes[box_idx * code_size + 2];
    float dy = gt_boxes[box_idx * code_size + 3];
    // origin dx/dy/dz is for box_encodings
    float origin_dx = gt_boxes[box_idx * code_size + 2];
    float origin_dy = gt_boxes[box_idx * code_size + 3];
    float cls = 0;
    if (code_size == 10) {
        
        cls = gt_boxes[box_idx * code_size + 9];
    } else if (code_size == 5) {
        cls = gt_boxes[box_idx * code_size + 4];
    } else {
        return;
    }
    ...
    gt_mask += cls_idx * max_objs;
    gt_cat += cls_idx * max_objs;
    gt_box_encoding += cls_idx * max_objs * 2;
    gt_cnt += cls_idx;
    ...
    gt_mask[obj_idx] = 1;
    gt_cat[obj_idx] = cls_idx + 1; // begin from 1
    gt_box_encoding[obj_idx * 2 + 0] = coor_x - coor_x_int;
    gt_box_encoding[obj_idx * 2 + 1] = coor_y - coor_y_int;
    return;
}

void draw_aux_kernel_launcher(int batch_size, int max_boxes, int max_objs, int num_cls, int H, int W, int code_size, int min_radius,
                                    float voxel_x, float voxel_y, float range_x, float range_y, float out_factor, float gaussian_overlap,
                                    const float *gt_boxes, float *heatmap, int *gt_ind, int *gt_mask, int *gt_cat, float *gt_box_encoding, int *gt_cnt){
    cudaError_t err;
    ...
    dim3 blocks(batch_size);
    dim3 threads(THREADS_PER_BLOCK);
    draw_aux_kernel<<<blocks, threads>>>(batch_size, max_boxes, max_objs, num_cls, H, W, code_size, min_radius,
                                            voxel_x, voxel_y, range_x, range_y, out_factor, gaussian_overlap,
                                            gt_boxes, heatmap, gt_ind, gt_mask, gt_cat, gt_box_encoding, gt_cnt);
