...
def remove_box_with_false_sizes(boxes, size_range, class_labels):
    """
    Args:
        boxes: (N, 7) [x, y, z, dx, dy, dz, heading, ...], (x, y, z) is the box center
        limit_range: [minx, miny, minz, maxx, maxy, maxz]
    ...
    for i in range(len(class_labels)):
        mask_i = (box_labels == class_labels[i]) & (boxes[:, [3, 4, 5]]>=size_range[i][0:3]).sum(axis=1) & (boxes[:, [3, 4, 5]]<=size_range[i][3:]).sum(axis=1)
...
def corners_nd(dims, origin=0.5):
    ...
    if ndim == 2:
        ...
        # corners_norm = corners_norm[[2, 3, 1, 0]]
    elif ndim == 3:
