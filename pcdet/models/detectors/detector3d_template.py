...
class Detector3DTemplate(nn.Module):
    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if cls_preds.shape[0] == 0:
                ...
                final_labels = batch_dict['batch_pred_labels'][batch_mask]
            else:
                if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                    ...
                    if batch_dict.get('has_class_labels', False):
                        ...
                        label_preds = batch_dict[label_key][batch_mask]
                    else:
    ...
    def load_first_stage_network_params_from_file(self, filename, logger, to_cpu=False):
        if not os.path.isfile(filename):
            raise FileNotFoundError

        logger.info('==> Loading first stage network parameters from checkpoint %s to %s' % (filename, 'CPU' if to_cpu else 'GPU'))
        loc_type = torch.device('cpu') if to_cpu else None
        first_stage_network_checkpoint = torch.load(filename, map_location=loc_type)['model_state']
        first_stage_para_for_second = {}
        if 'version' in first_stage_network_checkpoint:
            logger.info('==> Checkpoint trained from version: %s' % first_stage_network_checkpoint['version'])
        for key, val in first_stage_network_checkpoint.items():
            if key == 'global_step':
                continue
            first_stage_para_for_second[f'{key}'] = val
        self.load_state_dict(first_stage_para_for_second, strict=False)
        
        logger.info('Init first stage network successed!')
